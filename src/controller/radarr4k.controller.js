const debug = require('debug')('medidbutler:radarr4kController')
...
let settings = settingsService.getSettings('radarr4k') || false;

const setupNotifier = () => {
	const service = new radarrService(settings);
	service.getNotifiers().then((notifiers) => {
		const notificationUrl = (settingsService.settings.urlOverride) ? `${settingsService.settings.urlOverride}hooks/radarr4k/` : `http://${host}:${process.env.PORT || 9876}/hooks/radarr4k/`;
...
try {
	...
	console.log('Unable to load Radarr4K module. Possibly due to misconfiguration of settings. Enable debug logging for true output'); 
	debug(err);
 }
...
module.exports = {
	...
	saveConfigure: async (req, res, next) => {
		try {
			...
			if (r) {
				...
				allSettings['radarr4k'] = tempSettings;
	...
	hookRadarr: async (req, res, next) => {
		try {
			notificationService.emit('radarr4k', req.body);
