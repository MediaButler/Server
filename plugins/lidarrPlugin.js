...
const lidarrService = require('../service/lidarrService');
...
module.exports = class radarrPlugin extends basePlugin {
    ...
    async startup() {
        try {
            this.service = new lidarrService(this.settings);
    ...
    async api() {
        ...
        router.get('/lookup', async (req, res) => {
            try {
                const r = await this.service.lookupArtist({ name: req.query.query });
                if (!r) return res.status(200).send([]);
                return res.status(200).send(r);
            } catch (err) { return res.status(500).send({ name: err.name, message: err.message }); }
        });
        ...
        router.get('/:id', async (req, res) => {
            try {
                const r = await this.service.getArtist(req.params.id);
        ...
        router.get('/', async (req, res) => {
            try {
                const r = await this.service.getArtists();
                if (!r) return res.status(200).send([]);
                return res.status(200).send(r);
            } catch (err) { return res.status(500).send({ name: err.name, message: err.message }); }
        });
    ...
    async hasItem(musicBrainzId) {
        try {
            const r = await this.service.getArtistByMusicBrainzId(musicBrainzId);
            if (r) return true;
            else return false;
